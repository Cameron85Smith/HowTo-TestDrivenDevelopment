...
A developer writes production code and delivers the product, but now there is a bug in production. The developer gets back to the code and fixes the bug. **He then writes a unit test to ensure that the bug does not come back**, and the circle of hell repeats itself indefinitely.